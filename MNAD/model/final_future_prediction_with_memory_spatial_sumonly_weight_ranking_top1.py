...
class convAE(torch.nn.Module):
    def __init__(self, n_channel =3,  t_length = 5, memory_size = 10, feature_dim = 512, key_dim = 512, temp_update = 0.1, temp_gather=0.1):
        ...
        position = torch.randn((256,256),requires_grad=True)
        self.position = torch.nn.Parameter(position)
        self.register_parameter('position',self.position)

    def forward(self, x, keys, memory_dis, train=True, get_memory_dis=False):
        position = self.position
        position = position.repeat(12,1,1)
        x = x + position
        fea, skip1, skip2, skip3 = self.encoder(x)
        if train:
            updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, memory_dis = self.memory(fea, keys, memory_dis, train, get_memory_dis)#fea：1*512*32*32 key:10*512
            output = self.decoder(updated_fea, skip1, skip2, skip3)
            
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, memory_dis
        
        #test
        else:
            updated_fea, keys, softmax_score_query, softmax_score_memory,query, top1_keys, keys_ind, compactness_loss, memory_dis = self.memory(fea, keys, memory_dis, train, get_memory_dis)
            output = self.decoder(updated_fea, skip1, skip2, skip3)
            
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, query, top1_keys, keys_ind, compactness_loss, memory_dis
