...
class Memory(nn.Module):
    ...
    def get_score(self, mem, query):
        bs, h,w,d = query.size()# h*w个d维查询
        m, d = mem.size()# m个d维内存条目

        score = torch.matmul(query, torch.t(mem))# b X h X w X m
        score = score.view(bs*h*w, m)# (b X h X w) X m

        score_query = F.softmax(score, dim=0)# 匹配概率V 记忆条目到每个查询的距离
        score_memory = F.softmax(score,dim=1)# 匹配概率W 查询到每个记忆条目的距离

        return score_query, score_memory
    
    def forward(self, query, keys, memory_dis, train=True, get_memory_dis=False):
        ...
        if train:
            ...
            # read
            updated_query, softmax_score_query,softmax_score_memory = self.read(query, keys, memory_dis, train)
            #update
            updated_memory, memory_dis = self.update(query, keys, memory_dis, train, get_memory_dis)
            
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, memory_dis
        ...
        #test
            ...
            #update
            updated_memory = keys
            memory_dis = memory_dis
               
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory, query_re, top1_keys,keys_ind, compactness_loss, memory_dis
        
        
    
    def update(self, query, keys, memory_dis, train, get_memory_dis):
        
        batch_size, h,w,dims = query.size() # b X h X w X d
        #my code
        m, d = keys.size()  # m个d维内存条目
        score = torch.matmul(query, torch.t(keys))  # b X h X w X m
        score = score.view(batch_size * h * w, m)  # (b X h X w) X m
        # 得到每个簇，离记忆条目最远的query的距离
        if get_memory_dis:
            _, score_indices_min = torch.topk(score, 1, dim=-1, largest=False) # b X h X w X 1
            score_indices_min = score_indices_min.reshape(batch_size*h*w) # (b X h X w)
            indices = torch.arange(0,batch_size*h*w,1)
            score_min = score[indices,score_indices_min]
            score_min = score_min.view(batch_size,h,w,1)
            for i in range(batch_size):
                dis_cat = torch.cat((memory_dis,score_min[i]), dim=-1)
                memory_dis,_ = torch.max(dis_cat,dim=-1) # h X w
                memory_dis = memory_dis.unsqueeze(2) # h X w X 1

        softmax_score_query = F.softmax(score, dim=0)  # 匹配概率V 记忆条目到每个查询的距离
        softmax_score_memory = F.softmax(score, dim=1)  # 匹配概率W 查询到每个记忆条目的距离
        ...
        return updated_memory.detach(), memory_dis.detach()
    ...
    def read(self, query, updated_memory ,memory_dis ,train):
        batch_size, h,w,dims = query.size() # b X h X w X d
        ...
        if train:
            ...
            updated_query = updated_query.permute(0,3,1,2) # b X 2d X h X w

        else:
            m, d = updated_memory.size()  # m个d维内存条目

            score = torch.matmul(query, torch.t(updated_memory))  # b X h X w X m
            score = score.view(batch_size * h * w, m)  # (b X h X w) X m
            ...
            # my code:to fix query. memory_dis: h X w X 1
            _, score_indices_min = torch.topk(score, 1, dim=-1, largest=False)  # (b X h X w) X 1
            score_indices_min = score_indices_min.view(batch_size*h*w) # (b X h X w)
            indices = torch.arange(0, batch_size * h * w, 1)
            score_min = score[indices,score_indices_min].view(batch_size, h, w, 1)  # b X h X w X 1
            _, softmax_score_indices_max = torch.topk(softmax_score_memory, 1, dim=1)  # b X h X w X 1
            softmax_score_indices_max = softmax_score_indices_max.view(batch_size * h * w, 1)  # b X h X w X 1
            memory_dis_reshape = memory_dis.repeat(batch_size, 1, 1, 1)  # b X h X w X 1

            query_indices = torch.cat((score_min,memory_dis_reshape), dim=-1).argmin(-1).view(batch_size * h * w)  # (b X h X w)
            memory_indices = softmax_score_indices_max[query_indices==1].squeeze(1)  # (b X h X w)

            query_reshape[query_indices==1] = updated_memory[memory_indices]  # 使用最近的记忆修改距离太远的特征
