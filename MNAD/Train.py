...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_dir', type=str, default='log_position', help='directory of log')
    ...
    # args.model_dir = exp_path + args.dataset_type + '/log/' + model_dir
    # args.m_items_dir = exp_path + args.dataset_type + '/log/' + m_items_dir
    ...
    m_dis = F.normalize(torch.zeros((32, 32, 1), dtype=torch.float), dim=-1).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        for j,(imgs) in enumerate(train_batch):
            ...
            # imgs[:, 0:12] = imgs[:, 0:12] + timing_signal
            imgs = Variable(imgs).cuda()

            outputs, _, _, m_items, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, m_dis = model.forward(imgs[:,0:12], m_items, m_dis, True, get_memory_dis)
    ...
    torch.save(m_dis, os.path.join(log_dir, 'dis.pt'))
